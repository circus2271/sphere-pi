...
// Copy this file to `_playerConfig.js` (it's gitignored) and edit.
//
...
const playerConfig = {
    ...
    playlists: [
        ...
        { name: 'AllDay', file: path.resolve(__dirname, './tracksAllDay.txt'), start: '07:00', end: '25:00' },
        // per-playlist override возможен: { ..., tableId: 'tblXXXX' }
    ],

    // 2 playlists (day + night, ночь до 03:00 следующих суток):
    // playlists: [
    //     { name: 'day',   file: path.resolve(__dirname, './tracksDay.txt'),   start: '07:00', end: '18:00' },
    //     { name: 'night', file: path.resolve(__dirname, './tracksNight.txt'), start: '18:00', end: '27:00' },
    // ],

    // Старый формат (целые часы) тоже поддерживается:
    // playlists: [
    //     { name: 'all', file: path.resolve(__dirname, './tracksAllDay.txt'), startHour: 5, endHour: 2 }
    // ],
    ...
    // ── WEB SERVER (веб-пульт) ────────────────
    // Порт веб-пульта. Переменная окружения PORT имеет приоритет.
    port: 3333,

    // ── VOLUME ────────────────────────────────
    // Стартовая громкость (0–100). Дальше регулируется с веб-пульта.
    volume: 80,

    // ── AIRTABLE STATS ────────────────────────
    baseId: '',
    tableId: '',  // общий для всех плейлистов; per-playlist tableId (если задан) побеждает
};
